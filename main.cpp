...
void print_solution(char *op, char *pos, int size) {
  int idx;
...
*/
void evaluate(char *op, char *pos, int size) {
  float res[6];
  char last_touched[6] = {-1,-1,-1,-1,-1,-1};
...
#if DEBUG
          printf("avoid div by zero %f / %f = %f\n", op1!=-1?res[idx]:numbers[idx], op2!=-1?res[idx+1]:numbers[idx+1], (op1!=-1?res[idx]:numbers[idx]) / (op2!=-1?res[idx+1]:numbers[idx+1]));
#endif
...
*/
void iterate(char *op, char *pos, char *used, char curr, int size) {
  if (curr >= size-1) {
...
int is_already_searched(int size) {
  char buffer[4*6+1] = {0};
  int length = 0;
...
*/
void rearrange(int pos, char *consumed, int size, int ignored) {
